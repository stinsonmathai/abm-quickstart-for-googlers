...
create-vms:  ##   Create and bootstrap GCE instances
	...
	@/bin/bash utils/abm-vm-for-wireless-bootstrap.sh ${PROJECT_ID} ${ZONE} ${MACHINE_TYPE_MASTER} ${VM_COUNT_MASTER} ${MACHINE_TYPE_WORKER} ${VM_COUNT_WORKER} ${ABM_VERSION}
...
prepare-hybrid-cluster:  ##   Copy a hybrid cluster manifest to the workstation
	...
	wget -O bmctl-workspace/hybrid-cluster-001/hybrid-cluster-001.yaml https://raw.githubusercontent.com/stinsonmathai/abm-quickstart-for-googlers/${BRANCH}/abm-clusters/hybrid-cluster-001.yaml
...
google-identity-login:  ##   Enable Google Identity Login
	@gcloud compute ssh root@abm-ws --zone ${ZONE} -- -o ProxyCommand='corp-ssh-helper %h %p' -ServerAliveInterval=30 -o ConnectTimeout=30 << EOF
	wget -O google-identity-login.yaml https://raw.githubusercontent.com/stinsonmathai/abm-quickstart-for-googlers/${BRANCH}/anthos-features/google-identity-login.yaml
